...
func TestHandleCompactTranscript(t *testing.T) {
	...
	err := HandleCompactTranscript([]string{"--session-ref", "/tmp/repo/.entire/tmp/roo/task123.json"}, &stdout, testTranscriptCompactor{
...
func TestHandlePrepareTranscript(t *testing.T) {
	...
	err := HandlePrepareTranscript([]string{"--session-ref", "/tmp/repo/.entire/tmp/roo/task123.json"}, preparer)
	if err != nil {
		t.Fatalf("HandlePrepareTranscript() error = %v", err)
	}
	if preparer.sessionRef != "/tmp/repo/.entire/tmp/roo/task123.json" {
